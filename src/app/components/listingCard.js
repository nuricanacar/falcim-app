...
import OfferButton from './offerbutton'; // Teklif verme butonu componentini ekliyoruz
...
export default function ListingCard({ listing }) {
  ...
  const isFortuneTeller = user ? user.isFortuneTeller : false;
  return (
    <>
      <div className="group relative">
        <div className="aspect-h-3 aspect-w-4 overflow-hidden rounded-lg bg-gray-100">
          <p className="text-gray-500"> Kategori: {listing.fortuneType}</p>
          <p className="text-gray-500"> Yükleyen: {listing.user ? `${listing.user.firstName} ${listing.user.lastName}` : 'Bilinmiyor'}</p>
      ...
      <Modal isOpen={isModalOpen} onClose={closeModal}>
        ...
        <p className="text-gray-900 font-bold mt-4">Fiyat: </p>
        <p className="text-gray-500">Kategori: {listing.fortuneType}</p>
        <p className="text-gray-500">Açıklama: {listing.question}</p>
        {isFortuneTeller ? (
          <OfferButton listingId={listing.id} userId={user.id
          } />
        ) : (
          ""
        )}
